...
$AZ_CMD container create --resource-group $GROUPREPOSITORY --name $APP_IMAGE --image $ACR_NAME.azurecr.io/$APP_IMAGE:$TAG --cpu 1 --memory 1 --registry-login-server $ACR_NAME.azurecr.io --registry-username $USER_NAME --registry-password $PASSWORD --ip-address Public --dns-name-label dns-um-$RANDOM --ports 5000
